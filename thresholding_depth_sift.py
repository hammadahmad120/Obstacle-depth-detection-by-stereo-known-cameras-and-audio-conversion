...
if __name__ == '__main__':
    ...
    for i in range(distance.size):
        threshhold_values = ((distance > (distance[i] - 0.2)) & (distance < (distance[i] + 0.2))).sum()
        a = distance[threshhold_values]
        print(threshhold_values)
        if threshhold_values >= 3:
    ...
    msg = 'Obstacle distance ' + str("{:.2f}".format(min_distance)) + ' meter'
